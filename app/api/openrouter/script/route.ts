...
const SYSTEM_PROMPT = `You are a professional screenwriter. Given a logline, beat sheet, and treatment, write a short film script in proper screenplay format.

Use standard screenplay conventions:
...
Write for a 5-10 minute short film. Include all key beats from the beat sheet.`;
...
export async function POST(req: NextRequest) {
  ...
  const { logline, beatSheet, treatment, clientId, clientApiKey } = p.data;
  ...
  return NextResponse.json({ script: content, usage: j?.usage });
}
